...
const FinanceDashboard = () => {
  ...
  const chartTextAndStrokeColor = "#9ca3af"; // Tailwind gray-400 para tema oscuro
  const chartGridColor = "#374151";      // Tailwind gray-700 para tema oscuro
  // Paleta dorada/elegante para PieChart
  const PIE_CHART_COLORS = ['#facc15', '#fbbf24', '#f59e0b', '#d97706', '#b45309', '#78350f']; // yellow-500, amber-500, amber-600, amber-700, amber-800, amber-900
      ...
      <div className="min-h-screen bg-slate-900 flex items-center justify-center p-4 font-sans">
        <div className="bg-slate-800/70 backdrop-blur-md rounded-2xl p-8 w-full max-w-md border border-yellow-500/30 shadow-2xl shadow-yellow-500/20">
          <div className="text-center mb-10">
            <div className="bg-gradient-to-br from-yellow-400 via-yellow-500 to-amber-500 p-4 rounded-full w-20 h-20 mx-auto mb-6 flex items-center justify-center ring-2 ring-yellow-300/70">
              {logoUrl ? <img src={logoUrl} alt="Logo Estamos Benditos" className="h-12 w-12 rounded-full object-cover" /> : <DollarSign className="text-slate-900 text-3xl" />}
            </div>
          ...
          <div className="space-y-6">
            <div>
              ...
              <div className="relative">
                <User className="absolute left-3.5 top-1/2 -translate-y-1/2 text-yellow-400/80" size={20} />
            ...
            <div>
              ...
              <div className="relative">
                <Lock className="absolute left-3.5 top-1/2 -translate-y-1/2 text-yellow-400/80" size={20} />
                ...
                <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3.5 top-1/2 -translate-y-1/2 text-yellow-400/80 hover:text-yellow-300">
        ...
        <div className="max-w-full mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            <div className="flex items-center space-x-4">
              ...
              <button onClick={handleLogout} className="bg-red-600 text-white px-3 py-1.5 rounded-md hover:bg-red-700 transition-colors text-xs font-medium shadow-md hover:shadow-lg">
                Cerrar Sesión
              </button>
            </div>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <div className="bg-slate-800 rounded-xl shadow-xl p-6 border-l-4 border-green-500 hover:shadow-green-500/20 transition-shadow duration-300">
            <div className="flex items-center justify-between"><div><p className="text-sm font-medium text-slate-400">Ingresos Totales (COP)</p><p className="text-2xl font-bold text-green-400">{formatCurrency(totalIncome, 'COP')}</p></div><TrendingUp className="text-green-500" size={36} /></div>
          </div>
          <div className="bg-slate-800 rounded-xl shadow-xl p-6 border-l-4 border-red-500 hover:shadow-red-500/20 transition-shadow duration-300">
            <div className="flex items-center justify-between"><div><p className="text-sm font-medium text-slate-400">Gastos Totales (COP)</p><p className="text-2xl font-bold text-red-400">{formatCurrency(totalExpenses, 'COP')}</p></div><TrendingDown className="text-red-500" size={36} /></div>
          </div>
          <div className={`bg-slate-800 rounded-xl shadow-xl p-6 border-l-4 ${balance >= 0 ? 'border-yellow-500 hover:shadow-yellow-500/20' : 'border-orange-500 hover:shadow-orange-500/20'} transition-shadow duration-300`}>
            <div className="flex items-center justify-between"><div><p className="text-sm font-medium text-slate-400">Balance (COP)</p><p className={`text-2xl font-bold ${balance >= 0 ? 'text-yellow-400' : 'text-orange-400'}`}>{formatCurrency(balance, 'COP')}</p></div><DollarSign className={balance >= 0 ? 'text-yellow-500' : 'text-orange-500'} size={36} /></div>
          </div>
          <div className="bg-slate-800 rounded-xl shadow-xl p-6 border-l-4 border-purple-500 hover:shadow-purple-500/20 transition-shadow duration-300">
            <div className="flex items-center justify-between"><div><p className="text-sm font-medium text-slate-400">Transacciones (Filtradas)</p><p className="text-2xl font-bold text-purple-400">{filteredTransactions.length}</p></div><CreditCard className="text-purple-500" size={36} /></div>
          </div>
        </div>
        
        <div className="bg-slate-800 rounded-xl shadow-lg p-6 mb-8 border border-slate-700">
          <h3 className="text-xl font-bold text-yellow-400 mb-4 flex items-center"><Settings size={24} className="mr-3 text-yellow-500" /> Configuración de Moneda</h3>
          <div className="flex flex-col sm:flex-row items-end gap-4">
            <div className="flex-grow w-full sm:w-auto"><label htmlFor="exchangeRate" className="block text-sm font-medium text-slate-300 mb-1">Tasa de Cambio (1 USD = X COP):</label><input type="number" id="exchangeRate" value={tempExchangeRate} onChange={(e) => setTempExchangeRate(e.target.value)} className="px-3 py-2 bg-slate-700 border border-slate-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500 text-slate-100 w-full placeholder-slate-400" placeholder="Ej: 4000" /></div>
            <button onClick={handleSetExchangeRate} className="bg-yellow-500 text-slate-900 px-6 py-2 rounded-lg hover:bg-yellow-400 transition-colors font-semibold w-full sm:w-auto shadow-md hover:shadow-lg">Actualizar Tasa</button>
          </div><p className="text-xs text-slate-400 mt-2">Tasa actual: 1 USD = {exchangeRateUSDToCOP} COP</p>
        </div>
        
        <div className="bg-slate-800 rounded-xl shadow-lg p-6 mb-8 border border-slate-700">
          <h3 className="text-xl font-bold text-yellow-400 mb-6">Saldos por Bolsillo (COP)</h3>
          {pockets.length === 0 && pocketBalances.length === 0 ? (<p className="text-slate-400 text-center py-4">No has creado bolsillos.</p>) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
              {pocketBalances.map(pb => (<div key={pb.id} className={`rounded-lg p-4 shadow-lg ${pb.balance >= 0 ? 'bg-slate-700/50 border-l-4 border-sky-500' : 'bg-slate-700/50 border-l-4 border-orange-500'} hover:shadow-yellow-500/10 transition-all`}><h4 className="text-lg font-semibold text-yellow-400 truncate" title={pb.name}>{pb.name}</h4><p className={`text-2xl font-bold mt-1 mb-2 ${pb.balance >= 0 ? 'text-sky-300' : 'text-orange-300'}`}>{formatCurrency(pb.balance, 'COP')}</p><div className="text-xs mt-2 pt-2 border-t border-slate-600 space-y-1"><p className="text-green-400">Ingresos: {formatCurrency(pb.income, 'COP')}</p><p className="text-red-400">Gastos: {formatCurrency(pb.expenses, 'COP')}</p></div></div>))}
            </div>
          )}
        </div>
        ...
        <div className="flex flex-wrap gap-x-6 gap-y-4 items-center">
            <div className="flex items-center space-x-2 text-slate-300"><Filter size={20} className="text-yellow-400" /><span className="font-medium">Filtros:</span></div>
            <select value={dateFilter} onChange={(e) => setDateFilter(e.target.value)} className="px-3 py-2 bg-slate-700 border border-slate-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500 text-slate-100"><option value="all">Todas las fechas</option> <option value="7days">Últimos 7 días</option> <option value="30days">Últimos 30 días</option></select>
            <select value={categoryFilter} onChange={(e) => setCategoryFilter(e.target.value)} className="px-3 py-2 bg-slate-700 border border-slate-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500 text-slate-100"><option value="all">Todas las categorías</option> <option value="Salario">Salario</option> <option value="Freelance">Freelance</option> <option value="Vivienda">Vivienda</option> <option value="Alimentación">Alimentación</option> <option value="Transporte">Transporte</option></select>
          </div>
        </div>
          ...
          <div className="bg-slate-800 rounded-xl shadow-lg p-6 border border-slate-700">
            ...
            <ResponsiveContainer width="100%" height={300}>
              <LineChart data={chartData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                ...
                <Tooltip contentStyle={{ backgroundColor: 'rgba(30, 41, 59, 0.9)', border: `1px solid ${chartGridColor}`, borderRadius: '0.5rem' }} itemStyle={{ color: '#e2e8f0' }} labelStyle={{ color: '#fde047', fontWeight: 'bold' }} formatter={(value) => formatCurrency(value, 'COP')} />
                <Legend wrapperStyle={{ color: chartTextAndStrokeColor, fontSize: 12 }} />
                <Line type="monotone" dataKey="income" stroke="#34d399" strokeWidth={3} name="Ingresos" dot={{ r: 4, fill: "#34d399", stroke: chartGridColor }} activeDot={{ r: 7, stroke: '#fde047', strokeWidth: 2 }} />
                <Line type="monotone" dataKey="expenses" stroke="#f87171" strokeWidth={3} name="Gastos" dot={{ r: 4, fill: "#f87171", stroke: chartGridColor }} activeDot={{ r: 7, stroke: '#fde047', strokeWidth: 2 }}/>
              </LineChart>
            </ResponsiveContainer>
          </div>
          <div className="bg-slate-800 rounded-xl shadow-lg p-6 border border-slate-700">
            ...
            <ResponsiveContainer width="100%" height={300}>
              <PieChart margin={{ top: 0, right: 0, left: 0, bottom: 20 }}>
                <Pie data={pieData} cx="50%" cy="45%" labelLine={false} outerRadius={100} fill="#8884d8" dataKey="value"
        ...
        <div className="bg-slate-800 rounded-xl shadow-lg p-6 mb-8 border border-slate-700">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {goals.length === 0 && !showAddGoalForm && (<p className="text-slate-400 col-span-full text-center py-4">No has agregado ninguna meta.</p>)}
            {goals.map(goal => { 
                ...
                return (
                  <div key={goal.id} className="border border-slate-700 rounded-lg p-4 shadow-lg bg-slate-800/50 hover:border-yellow-500/50 transition-all">
                    <div className="flex items-center justify-between mb-2"><h4 className="text-lg font-semibold text-yellow-400">{goal.name}</h4><div className="flex items-center space-x-2"><Target className="text-sky-400" size={20} /><button onClick={() => handleDeleteGoal(goal.id)} className="text-red-500 hover:text-red-400" title="Eliminar meta"><Trash2 size={18} /></button></div></div>
                    <div className="space-y-2 mb-3"><div className="flex justify-between text-sm text-slate-300"><span>{formatCurrency(goal.currentAmount, 'COP')}</span><span>{formatCurrency(goal.targetAmount, 'COP')}</span></div><div className="w-full bg-slate-700 rounded-full h-4"><div className="bg-gradient-to-r from-sky-500 via-teal-500 to-emerald-500 h-4 rounded-full text-xs text-slate-800 font-medium flex items-center justify-center" style={{ width: `${Math.min(progress, 100)}%` }}>{progress > 15 && `${progress.toFixed(0)}%`}</div></div></div>
        ...
        <div className="bg-slate-800 rounded-xl shadow-lg p-6 mb-8 border border-slate-700">
          <div className="flex justify-between items-center mb-4"><h3 className="text-xl font-bold text-yellow-400">Mis Bolsillos</h3><button onClick={() => setShowAddPocketForm(!showAddPocketForm)} className="bg-yellow-500 text-slate-900 px-4 py-2 rounded-lg hover:bg-yellow-400 transition-colors flex items-center space-x-2 font-semibold shadow-md hover:shadow-lg">{showAddPocketForm ? <Minus size={20} /> : <FolderPlus size={20} />} <span>{showAddPocketForm ? 'Cancelar' : 'Nuevo Bolsillo'}</span></button></div>
          {showAddPocketForm && (<div className="mb-4 p-4 border border-slate-700 rounded-lg bg-slate-800/50"><h4 className="text-lg font-semibold text-yellow-300 mb-3">Crear Nuevo Bolsillo</h4><div className="flex items-end gap-4"><input type="text" placeholder="Nombre del bolsillo" value={newPocketName} onChange={(e) => setNewPocketName(e.target.value)} className="flex-grow px-3 py-2 bg-slate-700 border border-slate-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500 text-slate-100 placeholder-slate-400"/><button onClick={handleAddPocket} className="bg-sky-500 text-white px-6 py-2 rounded-lg hover:bg-sky-400 transition-colors font-semibold shadow-md hover:shadow-lg">Guardar Bolsillo</button></div></div>)}
          {pockets.length > 0 && (<div><h4 className="text-md font-semibold text-slate-300 mb-2 mt-4">Bolsillos existentes:</h4><ul className="space-y-2">{pockets.map(pocket => ( <li key={pocket.id} className="text-slate-200 pl-4 p-2 bg-slate-700/40 rounded-md border border-slate-600 hover:border-yellow-500/50 transition-colors">{pocket.name}</li> ))}</ul></div>)}
        ...
        <div className="bg-slate-800 rounded-xl shadow-lg p-6 mb-8 border border-slate-700">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 items-end">
            <select value={newTransaction.type} onChange={(e) => setNewTransaction({...newTransaction, type: e.target.value})} className="px-3 py-2.5 bg-slate-700 border border-slate-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500 text-slate-100"><option value="expense">Gasto</option><option value="income">Ingreso</option></select>
            <select value={newTransaction.currency} onChange={(e) => setNewTransaction({...newTransaction, currency: e.target.value})} className="px-3 py-2.5 bg-slate-700 border border-slate-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500 text-slate-100"><option value="COP">COP</option><option value="USD">USD</option></select>
            <input type="number" placeholder="Monto" value={newTransaction.amount} onChange={(e) => setNewTransaction({...newTransaction, amount: e.target.value})} className="px-3 py-2 bg-slate-700 border border-slate-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500 text-slate-100 placeholder-slate-400"/>
            <input type="text" placeholder="Categoría" value={newTransaction.category} onChange={(e) => setNewTransaction({...newTransaction, category: e.target.value})} className="px-3 py-2 bg-slate-700 border border-slate-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500 text-slate-100 placeholder-slate-400"/>
            <input type="text" placeholder="Descripción (Opcional)" value={newTransaction.description} onChange={(e) => setNewTransaction({...newTransaction, description: e.target.value})} className="px-3 py-2 bg-slate-700 border border-slate-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500 text-slate-100 placeholder-slate-400"/>
            <input type="date" value={newTransaction.date} onChange={(e) => setNewTransaction({...newTransaction, date: e.target.value})} className="px-3 py-2.5 bg-slate-700 border border-slate-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500 text-slate-100"/>
            <select value={newTransaction.selectedPocketId} onChange={(e) => setNewTransaction({...newTransaction, selectedPocketId: e.target.value})} className="px-3 py-2.5 bg-slate-700 border border-slate-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500 text-slate-100">
                ...
                {filteredTransactions.slice(-10).reverse().map(transaction => { 
                    ...
                    return (
                      <tr key={transaction.id} className="border-b border-slate-700 hover:bg-slate-700/50">
                        ...
                        <td className="py-3 px-4"><span className={`px-2 py-1 rounded-full text-xs font-medium ${ transaction.type === 'income' ? 'bg-green-800 text-green-200 border border-green-600' : 'bg-red-800 text-red-200 border border-red-600' }`}>{transaction.type === 'income' ? 'Ingreso' : 'Gasto'}</span></td>
